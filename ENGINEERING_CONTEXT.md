# ENGINEERING_CONTEXT.md

## Atlas Living Operational Truth

| Field | Value |
|-------|-------|
| **Repository** | Donum Dei Atlas — Repository 1 |
| **Classification** | Knowledge Preservation Repository |
| **Last Updated** | 2026-07-08 |
| **Living document** | Update at session end when state materially changes |

> **AI / engineer boot:** [PROJECT_CONTEXT.md](./PROJECT_CONTEXT.md) first ([DDES-001](../TradingBot/docs/ENGINEERING-STANDARDS/DDES-001-project-context-standard.md)), then [ARCHITECTURE.md](./ARCHITECTURE.md), then this file + [MISSION.md](./MISSION.md).

---

## 1. Current state

| Field | Value |
|-------|-------|
| **Stage** | Foundation — Repository 1 established |
| **Phase** | ATLAS-001 complete |
| **Active milestone** | [ATLAS-EM-001](./reports/ATLAS-EM-001-atlas-foundation-repository-established.md) — Published |
| **Parent gate** | [EM-007](../TradingBot/docs/engineering-milestones/EM-007-repository-0-approved-for-inheritance.md) — Satisfied |
| **Production impact** | None — documentation only |

Atlas is the first official inheriting repository of the Donum Dei Engineering Ecosystem.

---

## 2. Repository identity

| Field | Value |
|-------|-------|
| **Name** | Donum Dei Atlas |
| **Repository folder** | `DonumDei-Atlas` |
| **Classification** | Repository 1 |
| **Parent** | Repository 0 — [Donum Dei Engineering Bridge](../TradingBot) |
| **Mission** | Preserve, connect, and make discoverable institutional knowledge |

Atlas is **not** a product repository.

---

## 3. Inheritance status

Verified per [ATLAS-INHERITANCE-GUIDE](../TradingBot/docs/ENGINEERING-STANDARDS/ATLAS-INHERITANCE-GUIDE.md):

| Check | Result |
|-------|--------|
| EM-007 published | ✅ |
| Constitutional references (no duplication) | ✅ |
| Atlas `PROJECT_CONTEXT.md` per DDES-001 | ✅ |
| No DDTP product conflation in living docs | ✅ |
| DDES-002 / DDES-003 linked | ✅ |
| DDEV-000 / lexicon authority acknowledged | ✅ |
| DECS-001 readable via Repository 0 | ✅ |

**Inherit method:** Documented sibling reference to `../TradingBot` until physical DDEB split.

---

## 4. What Atlas owns

| Domain | Status at foundation |
|--------|----------------------|
| Knowledge organization | Structure established — [docs/knowledge-domains/](./docs/knowledge-domains/) |
| Knowledge relationships | Architecture defined — [ARCHITECTURE.md](./ARCHITECTURE.md) |
| Knowledge preservation | Philosophy and rules established |
| Knowledge discovery | Roadmap Phase 3 planned |
| Atlas engineering milestones | ATLAS-EM-001 published |
| Session evidence | `reports/` initialized |

---

## 5. Constitutional references (not duplicated)

| Artifact | Repository 0 path |
|----------|-------------------|
| The Donum Dei Way | [docs/THE-DONUM-DEI-WAY.md](../TradingBot/docs/THE-DONUM-DEI-WAY.md) |
| DDEF Principles | [DDEF-PRINCIPLES.md](../TradingBot/docs/engineering-framework/DDEF-PRINCIPLES.md) |
| DDEOS Foundation | [DDEOS-FOUNDATION-v1.0.md](../TradingBot/docs/engineering-framework/DDEOS-FOUNDATION-v1.0.md) |
| DDES-001–003 | [docs/ENGINEERING-STANDARDS/](../TradingBot/docs/ENGINEERING-STANDARDS/) |
| DDEV-000 | [DDEV-000](../TradingBot/docs/ENGINEERING-STANDARDS/DDEV-000-engineering-vocabulary-authority.md) |
| ENGINEERING-LEXICON | [docs/ENGINEERING-LEXICON.md](../TradingBot/docs/ENGINEERING-LEXICON.md) |
| DECS library | [docs/engineering-case-studies/](../TradingBot/docs/engineering-case-studies/) |
| ATLAS-INHERITANCE-GUIDE | [ATLAS-INHERITANCE-GUIDE](../TradingBot/docs/ENGINEERING-STANDARDS/ATLAS-INHERITANCE-GUIDE.md) |

---

## 6. Ecosystem relationships

```text
Repository 0  →  constitutional engineering authority
Atlas         →  knowledge preservation (this repository)
DDTP          →  engineering experience producer
Future repos  →  inherit Repository 0 + Atlas knowledge patterns
```

Trading Platform experience flows to Atlas through **documented curation** — not automatic code sync.

---

## 7. Recent milestones

| Date | ID | Title |
|------|-----|-------|
| 2026-07-08 | **ATLAS-EM-001** | Atlas Foundation Repository Established |
| 2026-07-08 | EM-007 (parent) | Repository 0 Approved for Inheritance |

---

## 8. Open work (non-blocking)

| Item | Owner | Phase |
|------|-------|-------|
| Domain content curation | Atlas | Phase 2 |
| Experience intake protocol | Atlas | Phase 2 |
| Discovery conventions | Atlas | Phase 3 |
| Physical DDEB split | Founder | Deferred |
| Atlas DECS when earned | Atlas | Future |

---

## 9. Repository health

| Area | Status |
|------|--------|
| Documentation structure | ✅ Established |
| Constitutional inheritance | ✅ Verified |
| Knowledge domains | 🟡 Scaffolded — content pending |
| Reports / evidence | ✅ ATLAS-EM-001 published |
| Production code | ✅ None (by design) |
| Dependencies | ✅ None (by design) |
| Automation | ✅ None (by design) |

---

## 10. Session protocol

Every Atlas engineering session:

1. Load [PROJECT_CONTEXT.md](./PROJECT_CONTEXT.md)
2. Load [ARCHITECTURE.md](./ARCHITECTURE.md)
3. Load this file + [MISSION.md](./MISSION.md)
4. Confirm priorities and open work
5. Begin knowledge work
6. Update living docs if state materially changed

---

*Atlas Engineering Context · Ad Majorem Dei Gloriam.*
